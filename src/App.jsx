    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="search-box">
          ...
          <div style={{fontSize:12,color:'#555'}}>Use location search box on the map to fly to places</div>
        </div>
        <ul style={{listStyle:'none',padding:0}}>
          {LOCATIONS.map(loc => (
            <li key={loc.id} className="location-item" onClick={() => handleListClick(loc)}>
      ...
      <div className="map-wrap">
        <MapContainer
          whenCreated={(m) => { mapRef.current = m; addSearch(m) }}
        ...
        >
          ...
          {LOCATIONS.map(loc => {
            const icon = loc.icon ? L.icon({ iconUrl: loc.icon, iconSize: [36,36], iconAnchor: [18,36] }) : undefined
            return (
          ...
          {PATHS.map((coords, i) => (
            <Polyline key={i} positions={coords} pathOptions={{color:'blue', weight:4, opacity:0.7}} />
          ))}
